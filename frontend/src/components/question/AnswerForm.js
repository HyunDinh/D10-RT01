            ...
            return (
              <div key={a.answerId} className="border rounded p-3 mb-3 bg-light">
                ...
                {editingAnswerId === a.answerId ? (
                  <>
                    <textarea className="form-control mb-2" value={editContent} onChange={e => setEditContent(e.target.value)} rows={3} />
                    <input type="file" className="form-control mb-2" accept="image/*" onChange={e => setEditImageFile(e.target.files[0])} />
                    <div className="d-flex gap-2 mb-2">
                      <button className="btn btn-success btn-sm" onClick={() => handleEditSave(a.answerId)} disabled={editLoading}>
                        {editLoading ? 'Đang lưu...' : 'Lưu'}
                      </button>
                      <button className="btn btn-secondary btn-sm" onClick={handleEditCancel} disabled={editLoading}>
                        Hủy
                      </button>
                    </div>
                  </>
                ...
                <div className="d-flex justify-content-between align-items-center mt-2">
                  <div className="text-end text-muted" style={{fontSize:'0.9em'}}>{a.createdAt ? new Date(a.createdAt).toLocaleString() : ''}</div>
                  {isOwner && editingAnswerId !== a.answerId && (
                    <div className="d-flex gap-2">
                      <button className="btn btn-outline-warning btn-sm" onClick={() => handleEditClick(a)}>
                        Sửa
                      </button>
                      <button className="btn btn-outline-danger btn-sm" onClick={() => handleDelete(a.answerId)} disabled={deletingId === a.answerId}>
